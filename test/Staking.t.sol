...
import {StakingRewards, IERC20} from "src/contract/Staking.sol";
import {MockERC20} from "test/token/MockErc20.sol";
...
contract StakingTest is Test {
    ...
    // function test_Notify_Rewards_When_TimestampIs_LessThan_FinishAt() public{
    //     // simulate owner calls setReward successfully
    //     vm.prank(owner);
    //     staking.setRewardsDuration(1 weeks);
    //     assertEq(staking.duration(), 1 weeks, "duration not updated correctly");
    ...
    //             staking.notifyRewardAmount(100 ether);
    //     // assertEq(staking.rewardRate(), uint256(300 ether)/uint256(1 weeks));
    ...
    // //     console.log("time warped to: ========1", block.timestamp);

    // //     console.log("finish at is ========", staking.finishAt());
    ...
    // }
      ...
      function test_getReward() public {
        ...
        // Tell staking how to distribute rewards
        staking.notifyRewardAmount(100 ether);
        vm.stopPrank();
        ...
        vm.startPrank(bob);
        uint bobsBalanceBefore = IERC20(address(rewardToken)).balanceOf(bob);
        console.log("bobs balance before reward", bobsBalanceBefore);
        uint earned = staking.earned(bob);
        console.log("bobs earned reward is", earned);
        staking.getReward();
        uint bobsBalanceAfter = IERC20(address(rewardToken)).balanceOf(bob);
        console.log("bobs balance after reward", bobsBalanceAfter);
        assertEq(bobsBalanceAfter, bobsBalanceBefore + earned, "bobs balance didnt update correctly");
    }
